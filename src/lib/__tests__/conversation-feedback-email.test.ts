...
describe("conversation feedback email helpers", () => {
  it("builds five rating links and renders them in text and html", () => {
    const links = buildConversationFeedbackLinks("https://chatly.example", "conv_123");

    expect(links).toHaveLength(5);
    expect(links[0]).toEqual({
      rating: 1,
      label: "1 star",
      href: "https://chatly.example/feedback?conversationId=conv_123&rating=1"
    });
    expect(links[4]).toEqual({
      rating: 5,
      label: "5 stars",
      href: "https://chatly.example/feedback?conversationId=conv_123&rating=5"
    });

    expect(renderConversationFeedbackText(links)).toContain(
      "5 stars: https://chatly.example/feedback?conversationId=conv_123&rating=5"
    );
