...
import { PUBLISHING_NAV, type PublishingSection } from "./dashboard-publishing-types";

export function DashboardPublishingScaffold({
  activeSection,
  children
}: {
    ...
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
      <div className="border-b border-slate-200 bg-slate-50 px-4 py-4 lg:hidden">
        <div className="flex gap-2 overflow-x-auto pb-1">
          {PUBLISHING_NAV.flatMap((group) => group.items).map((item) => {
            const Icon = item.icon;
            return (
              <Link
                key={item.value}
                href={buildPublishingSectionHref(item.value)}
          ...
          <div className="px-3 py-5">
            {PUBLISHING_NAV.map((group) => (
              <div key={group.label} className="mt-2 first:mt-0">
                <p className="px-3 pb-2 text-[11px] font-medium uppercase tracking-[0.08em] text-slate-500">{group.label}</p>
